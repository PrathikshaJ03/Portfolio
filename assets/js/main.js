...
(function() {
   ...
   */
  window.addEventListener('load', () => {
    let resumeContainer = select('.resume-container');
    if (resumeContainer) {
      let resumeIsotope = new Isotope(resumeContainer, {
        itemSelector: '.resume-item'
      });

      let resumeFilters = select('#resume-flters li', true);

      on('click', '#resume-flters li', function(e) {
        e.preventDefault();
        resumeFilters.forEach(function(el) {
          el.classList.remove('filter-active');
        });
        this.classList.add('filter-active');

        resumeIsotope.arrange({
          filter: this.getAttribute('data-filter')
        });
        resumeIsotope.on('arrangeComplete', function() {
          AOS.refresh()
        });
      }, true);
    }
